        ...
        <div>
          ...
          <div class="headimg">
            <div>
              <div
                class="categoryWrap"
                :style="{backgroundImage:'url('+product.wap_banner_url + ')'}"
              >{{product.front_name}}</div>
            </div>
          </div>
          <!-- 标题 -->
          <h3>---{{product.name}}----</h3>
          ...
          <div class="allproduct">
            <dl
              v-for="(citem) in product.subCategoryList"
              :key="citem.id"
              @click="()=>todetail(citem.id)"
            >
              <dt>
...
export default Vue.extend({
  ...
  methods: {
    ...mapActions({
      getclassify: "getGoods/getclassify",
      getanother: "getGoods/getanother",
      getclass: "getGoods/getclass",
